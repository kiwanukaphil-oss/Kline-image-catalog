...
## Namugongo post-receipt audit and photo recovery

- [x] Read-only production audit: 67 lots became 38 products / 124 variants / 126 units. Current branch quantities match received quantities. Product/variant links, active assortment, mapped categories, prices, costs, EAN-13 barcodes, material, brand and variant attributes match the source or approved matching plan. Evidence: `verification/namugongo-received-product-audit.json`.
- [x] Confirm the defect: all 67 photo handoffs are failed; all 38 created products have no POS primary image or gallery. The sampled original photo remains readable. A deterministic copy of that photo into its intended POS storage key was created and verified during diagnosis; no product/gallery/stock records were changed.
- [x] Owner approved repair. Retried the native photo-only service: all 67 handoffs linked; all 38 products have exactly one primary matching their gallery; all 67 copied photos have gallery records. Repeated audit still reports 124 variants / 126 units and zero field or balance mismatches. Live POS Mack Weldon 26201A-7 renders both photos and switches correctly.
...
## Removed colours and barcode label selection

- [x] Diagnose: removing a dimension dropped its cells from the form, but the save diff never retired omitted variants. Labels selected every active variant, including zero-stock rows.
- [x] Owner approved correction. Retired only the two empty Light Grey variants of Gauche UOMO Formal Shirt - Black Button in an audited, backed-up transaction. White L remains 1 unit; White XL remains 2 units. All stock balances and barcode identities preserved. Evidence: `verification/gauche-grey-retirement.json`.
- [x] Verify live product overview, label dialog and edit page show only the White variants; original Light Grey adjustments remain in history.
- [x] Implement omitted-variant retirement after replacement creation; reject stocked removals before frontend writes and check all branch balances under database locks at the API. Labels default to stocked active variants, with stock counts visible and empty variants still manually selectable.
- [x] Five frontend regression tests and three PostgreSQL/API tests pass, including replacement failure, stock protection and stock held in another branch. TypeScript and production build pass.
