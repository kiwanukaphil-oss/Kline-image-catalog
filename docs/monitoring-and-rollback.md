...
Production upload verification completed after API deployment `5afadc04-6e5d-46a7-b26a-85bbc0575244` explicitly applied the checksum setting. Reapplying an unchanged variable did not trigger a deployment; a changed release marker did. Verify actual uploads after deployment rather than treating configured variable values or a healthy process as proof of runtime storage behavior. Post-fix GitHub health run `34157581100` passes all eight probes.

## Custom domain rollback

`https://klinemen-catalog.com` is the production address. The direct Railway URL remains available. Domain verification and HTTPS passed on 7 September 2026; eight local public probes passed against the custom domain. The updated probe is in the working tree; the existing GitHub workflow keeps using its committed version until the changes are committed and pushed.

For a separately authorized domain rollback, restore the apex to the four DNS-only A values in `verification/production-domain-cutover.json` (185.199.108.153 through 185.199.111.153). Three are also retained as annotated `previous-origin` DNS records. This requires replacing the apex CNAME; preserve its Railway target for restoring the new app. The old GitHub Pages deployment and existing www-to-apex redirect are retained. DNS rollback alone may leave an installed browser running its newer service worker, so verify the browser transition before declaring rollback complete. Do not clear site storage: it may contain unsent photos.
